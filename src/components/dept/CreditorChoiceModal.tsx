...
/** Pop-up เลือกงานของปุ่ม "ตั้งเจ้าหนี้" */
export function CreditorChoiceModal({ open, onClose }: Props) {
  const navigate = useNavigate();
  const receiveCount = 0;
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="creditor-choice-modal-title"
    >
      <button
        type="button"
      ...
      >
        <h2 id="creditor-choice-modal-title" className="sr-only">
          เลือกประเภทงานตั้งเจ้าหนี้
        </h2>

        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <div
            className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-violet-100/95 via-fuchsia-50/80 to-white py-8 pl-4 pr-8 shadow-sm ring-1 ring-violet-200/70"
            style={{
          ...
          >
            <div className="flex justify-center">
              <button
                ...
                onClick={() => {
                  onClose();
                  navigate("/dept/accounting/add-creditor");
                }}
                className="min-w-[10.5rem] rounded-2xl bg-violet-500 px-8 py-3 text-2xl font-bold text-white shadow-md transition hover:bg-violet-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-300 focus-visible:ring-offset-2"
              >
                เพิ่มเจ้าหนี้
              </button>
            </div>
          ...
          <div
            className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-rose-100/90 via-rose-50/85 to-white py-8 pl-4 pr-8 shadow-sm ring-1 ring-rose-200/70 sm:col-span-2 lg:col-span-1"
            style={{
              clipPath: "polygon(0 0, calc(100% - 1.8rem) 0, 100% 100%, 0 100%)",
            }}
          >
            <div className="flex justify-center">
              <button
                ...
                className="min-w-[10.5rem] rounded-2xl bg-rose-600 px-8 py-3 text-2xl font-bold text-white shadow-md transition hover:bg-rose-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-300 focus-visible:ring-offset-2"
              >
                รออนุมัติ
              </button>
            </div>
