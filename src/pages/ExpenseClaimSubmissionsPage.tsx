...
export function ExpenseClaimSubmissionsPage() {
  const { user } = useAuth();
  ...
  return (
    <div className="min-h-full bg-slate-100/80">
      <header className="border-b border-slate-200/80 bg-white px-6 py-4">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-900">ใบขอเบิก</h1>
      ...
      <div className="mx-auto max-w-xl p-6">
        {claims.length === 0 ? (
          ...
          <ul className="space-y-4">
            {claims.map((c, i) => (
              <li key={c.id}>
                <article className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-md ring-1 ring-slate-100/80">
