...
export function AccountingDeptReport() {
  ...
  const pieData =
    fromDb && acc?.pie && acc.pie.length > 0 ? acc.pie : [];
  ...
  return (
    <DeptPageFrame>
      ...
      <p className="mb-4 text-sm text-slate-600">
        ...
        {!loading && !fromDb && (
          <span>
            ยังไม่มีข้อมูลจากฐานข้อมูล — นำเข้า{" "}
      ...
      <div className="mx-auto max-w-4xl rounded-2xl border border-slate-200/90 bg-white p-6 shadow-sm ring-1 ring-slate-100/80 sm:p-8">
        <h2 className="text-center text-lg font-semibold text-slate-900">
          {pieData.length > 0
            ...
            : "สัดส่วนมูลค่าจากเอกสาร"}
        </h2>
        <p className="mt-1 text-center text-sm text-slate-500">
          {pieData.length > 0
            ? "รวมยอดใบเสนอราคา จำนวนเอกสาร และเงินล่วงหน้า — ใช้ดูภาพรวมเชิงปริมาณ"
            : "เมื่อมีข้อมูลใน bill_quota / receive_order / RFQ จะแสดงกราฟที่นี่"}
        </p>

        {pieData.length === 0 ? (
          <p className="mt-8 text-center text-sm text-slate-500">
            ไม่มีข้อมูลสำหรับแสดง — ตรวจสอบการนำเข้าฐานข้อมูล MySQL (mns_pm_2021)
          </p>
        ) : (
          <>
