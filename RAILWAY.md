...
โค้ดใน `server/db.mjs` ประกอบการเชื่อม MySQL ดังนี้:

1. ถ้ามี `DATABASE_URL` ที่ขึ้นต้นด้วย `mysql://` → **ใช้ค่านี้ก่อน**
2. ถ้าไม่มีหรือ `DATABASE_URL` ไม่ใช่ `mysql://` → ใช้ **`MYSQL*`** (Railway) ถ้าครบ ไม่ก็ใช้ **`DB_*`** (cPanel)

บริการเว็บบน Railway ควรได้ **`MYSQL*`** จากบริการ MySQL เดียวกัน (หรือใส่ `DATABASE_URL=mysql://...` เอง)

---

## 5) Prisma / ตารางส่วนขยาย (ถ้าคุณใช้งานส่วนนี้)

Prisma ใช้ **MySQL เท่านั้น** (`server/prisma/schema.prisma`) — อ่าน `DATABASE_URL` ผ่านสคริปต์ `server/scripts/prisma-with-db-env.mjs` ซึ่งประกอบ URL จาก **`MYSQL*` หรือ `DB_*`** เหมือนแอปหลัก

**ไม่มี `prisma migrate` ใน repo** — โครงฐานหลักมาจาก import SQL (`mns_pm_2021.sql` / `db_mns.sql` ฯลฯ) ตาม `database/IMPORT.txt` ถ้าต้องการให้ตารางตาม `schema.prisma` ตรงกับฐานที่รันอยู่ (เช่น ตาราง `mns_*`) ใช้ **`db push`** (ระวัง: แก้ schema บนฐานจริง):
...
หรือ `cd server` แล้ว `npm run db:push`

(ต้องมี `MYSQL*` หรือ `DATABASE_URL=mysql://...` ใน `server/.env` หรือ export ก่อนรัน)

สร้าง Prisma Client หลังแก้ schema: `npm run db:generate`
...
| `DATABASE_URL` ไม่ใช่ MySQL | ใช้เฉพาะ `mysql://` หรือลบออกแล้วให้มี **`MYSQL*`** ครบที่บริการเว็บ |
